...
$("#edit_sno").on("input",function () {
    checkEditSno()
});
function checkEditSno(){
    let $d = $("#edit_sno");
    let val = $d.val();
...
function checkInsertSno() {
    let $sno = $("#insert_sno")
    if($sno.val().trim() == "" || $sno.val().trim().length < 6){
        $sno.attr("class","form-control is-invalid")
        return false
    }
    $sno.attr("class","form-control is-valid")
    return true
}
...
function checkInsertAll() {
    ...
    let sno = checkInsertSno()
    let email = checkInsertEmail()
    let phone = checkInsertPhone()
    return name && account && sex && sno && birthday && email && phone
}
