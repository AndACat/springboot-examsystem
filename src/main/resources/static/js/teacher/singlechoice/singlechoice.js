$(function () {
    ...
    $("#jsGrid").jsGrid({
        ...
        controller: {
            loadData: function (filter) {
                let localData;
                $.ajax({
                    url: "/teacher/selectMyAllSingleChoice",
                    dataType: "json",
                    data: filter,
                    async: false,
    ...
    $(document).on("click",".openEditModal",function () {
        ...
        $("#edit_answer").val(data.answer.toUpperCase());
    ...
    $("#editProblem_server").click(function () {
        ...
        if(!checkProblemInfo()){
            ...
            $.ajax({
                url: "/teacher/updateSingleChoiceInfo",
                type: "post",
                processData: false,
                contentType: false,
                data: formData,
    ...
    $(document).on("click",".openDeleteConfirm",function () {
        if(confirm("确认删除此题吗?")){
            ...
            let uuid = data.uuid;
            $.ajax({
                url: "/teacher/deleteSingleChoiceByUuid",
                data: {data:uuid},
                type: "post",
                success: function (data) {
                    if(data.code == 200 || data.code == "200"){
    ...
    $("#insert_submitExcel").click(function () {
        ...
        if(file != undefined){
            $.ajax({
                url: "/teacher/insertSingleChoiceByExcel",
                data: formData,
                type: "post",
                processData: false,
                contentType: false,
    ...
    $("#saveProblem_server").click(function () {
        if(checkInsert_ALl()){
            ...
            $.ajax({
                url:"/teacher/insertSingleChoice",
                type:"post",
                processData: false,
                contentType: false,
                data: formData
...
function getEditProblemData() {
    ...
    let choice = $("#edit_answer").val().toLowerCase();
    for(let i=97;i<=104;++i){
        let c = String.fromCharCode(i)
...
function getInsertProblemData() {
    ...
    formData.append("singlechoice",JSON.stringify(problemData));
    return formData;
}
