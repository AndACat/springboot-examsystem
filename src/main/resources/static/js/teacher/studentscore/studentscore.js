...
let studentPaperScoreList = null;
$(function () {
    let $search1 = $("#search1");
    ...
    $paper_select_node.append("<select id='paperselect' class='form-control' style='width: 300px;'></select><button class='btn btn-info btn-sm' id='selectScore'>查成绩</button>");
    $.ajax({
        url: "/teacher/getScoreOfMyAllPaper",
        async: false,
        success:function (data) {
            let $paperselect = $("#paperselect");
    ...
    $(document).on("click","#selectScore",function () {
        ...
        $("#jsGrid").jsGrid({
            ...
            controller: {
                loadData: function (filter) {
                    let localData;
                    $.ajax({
                        url: "/teacher/getStudentScores",
                        dataType: "json",
                        data: {paper_uuid:paper.uuid},
                        async: false,
                        success: function (data) {
                            studentPaperScoreList = data;
                            localData = data;
                        }
                    });
                    return localData;
            ...
            fields: [{
                ...
                problemScoreConfigs[5] == "" ? defaultProblemScoreConfig : problemScoreConfigs[5],
                {
                    ...
                    itemTemplate: function (value, item) {
                        let str = '';
                        let zbList = item.zbList;
                        if(zbList.length == 0){
                            str += "<button class='btn btn-sm btn-primary disabled'>无作弊记录</button>"
                        }else{
                            str += "<button class='btn btn-sm btn-warning openModal' data='"+JSON.stringify(zbList)+"'>"+zbList.length+"条作弊记录</button>"
                        }

                        str += "<button class='showStudentPaperAnswer btn btn-sm btn-success' data='"+item.uuid+"'>查看答题详情</button>";
    ...
    $(document).on("click",".openModal",function () {
        let zbList = JSON.parse($(this).attr("data"));
        let $zbImgList = $("#zbImgList");
        $zbImgList.html("")
        for (const zbImgAddress of zbList) {
            $zbImgList.append("<img src='.."+zbImgAddress+"'>");
        }
        $("#modal").modal("show");
    })
    $(document).on("click",".showStudentPaperAnswer",function () {
        let uuid = $(this).attr("data");
        let studentScore = null;
        for (const studentPaperScoreElement of studentPaperScoreList) {
            if(studentPaperScoreElement.uuid == uuid){
                studentScore = studentPaperScoreElement;
                break;
            }
        }
        ...
        paper_uuid = studentScore.paper.uuid;
        let answers = studentScore["answers"];//eg: {1:"D",2:"B",3:"B",4:"B",5:"A"}
        let paperStrategyUuid = studentScore.paper.paperStrategy_uuid;
        let paperStrategy = null;
        $.ajax({
            url:"/teacher/selectPaperStrategyByPaperStrategy_uuid",
            data:{
                paperStrategy_uuid:paperStrategyUuid
            },
            async:false,
            success:function (data) {
                paperStrategy = data;
        ...
        let paperInfo = studentScore.paper.paperInfo;
        let markPlaceHtml = "";
            ...
            switch (problemType) {
                ...
                case "判断题":{
                    ...
                    $.each(problemInfoList,function (idx,itm) {//每道题
                        ...
                        judgeListBody+="<span class=\"text-red\">参考答案</span>&nbsp;&nbsp;&nbsp;<span>"+(itm.answer == true ? "对" : "错")+"</span><br>";
        ...
        $("#answerPlace").html(markPlaceHtml);
        $("#studentPaperAnswerModal").modal('show');
    })
})
