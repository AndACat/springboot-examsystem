$(function () {
    ...
    $(document).on("click",".openEditModal",function () {
        ...
        $("#edit_problem").val(getNoProcessProblem(data.problem,data.answer));
        $("#edit_answer").val(JSON.stringify(data.answer));
    ...
    $("#edit_problem").bind("input propertychange",function () {
        $("#edit_answer").val(getAnswer($(this).val()));
    })
...
function getEditProblemData() {
    ...
    problem.problem = getProcessProblem($("#edit_problem").val());
    problem.answer = JSON.parse($("#edit_answer").val());//getProcessProblem($("#edit_answer").val());
...
function getAnswer(problem) {
    ...
    for (let i = 0; i < problem.length; i++) {
        ...
        if(problem.charAt(i) == '}'){
            if(beginIdx != -1){
                let answer = problem.substring(beginIdx+1,i);
    ...
    return JSON.stringify(answerList);//.replace("#{}","___");
}
function getProcessProblem(problem){
    let beginIdx = -1;
    for (let i = 0; i < problem.length; i++) {
        if(problem.charAt(i) == '#' && problem.charAt(i+1) == '{'){
            i++;
        ...
        if(problem.charAt(i) == '}'){
            if(beginIdx != -1){
                problem = problem.substring(0,beginIdx+1) + problem.substring(i,problem.length);
                i = beginIdx;
                beginIdx = -1;
            }
        }
...
function getNoProcessProblem(problem, answerList) {//返回没有处理过的题目
    let answerIdx = 0;
    while(problem.includes("#{}")){
        let ans = answerList[answerIdx++];
        problem = problem.replace("#{}","#{"+ans+"}");
    }
    return problem;
}