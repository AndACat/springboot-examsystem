...
$("#edit_tno").on("input",function () {
    checkEditTno()
});
function checkEditTno(){
    let $d = $("#edit_tno");
    let val = $d.val();
...
function checkInsertTno() {
    let $tno = $("#insert_tno")
    if($tno.val().trim() == "" || $tno.val().trim().length < 6){
        $tno.attr("class","form-control is-invalid")
        return false
    }
    $tno.attr("class","form-control is-valid")
    return true
}
...
function checkInsertAll() {
    ...
    let tno = checkInsertTno()
    let email = checkInsertEmail()
    let phone = checkInsertPhone()
    return name && account && sex && tno && birthday && email && phone
}
